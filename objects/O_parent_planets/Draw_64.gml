...
if (global.open_gui && planet_id == id)
{
    ...
    for (var i = 0; i < array_length(_sprites); i++) {
        var textIndex = i; // Set "textIndex" initially to "i", meaning the current index of the loop (0, 1, 2, ...).
        switch (sprite_index[i]) { // changes the icon exibeted and the info 
            case Ice: textIndex = 0; break;
            case Terran1: textIndex = 1; break;
...
if (mouse_check_button_pressed(mb_left)) {
    ...
    if (clicked_button >= 0) {
        ...
        var actions = [
            [false, false, false], // play button - [mission1, mission2, mission3]
